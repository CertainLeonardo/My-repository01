...
int math(int x, int y, int z, int v) // set up parameters for the math function.
{
    // we will use the comparison greater than or equal to operator ">=" to produce a bool value 
    if (x >= y && z <= v) // why not also add a logical operater as well?
    {
        std::cout << "x is greater than or equal to y and z is less than or equal to v!" << std::endl;
...
int main()
{
    ...
    std::cin >> choice2;
    std::cout << "Pick another number: " << std::endl;
    std::cin >> choice3;
    std::cout << "Pick another number: " << std::endl;
    std::cin >> choice4;
    math(choice1, choice2, choice3, choice4);
